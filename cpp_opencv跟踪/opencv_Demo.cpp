...
int main()
{
    ...
    Mat frame, hsv, hue, mask, hist, histimg = Mat::zeros(300, 300, CV_8UC3), backproj;
    bool paused = false;
    ...
    while (true)
    {
        ...
        if (trackObject)                  //trackObject初始化为0,或者按完键盘的'c'键后也为0，当鼠标单击松开后为-1
        {
            ...
            rectangle(image, Point(trackWindow.x, trackWindow.y), Point(trackWindow.x + trackWindow.width, trackWindow.y + trackWindow.height), Scalar(0, 255, 0), 1, CV_AA);
        }
